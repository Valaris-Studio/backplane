...
function makeLoop(maxBlockedOnHuman: number) {
  return {
    enabled: false,
    provider: "",
    model: "mid",
    ...
    max_blocked_on_human: maxBlockedOnHuman,
...
// surface rule applies: it is editable from the Loop dialog AND via MCP.
describe("BoardLoopDialog — max_blocked_on_human", () => {
  it("renders the stored value in the rails section", async () => {
    server.use(http.get(LOOP_URL, () => HttpResponse.json(makeLoop(5))));
    renderDialog();

    const field = await screen.findByLabelText(/blocked on human/i);
    await waitFor(() => expect(field).toHaveValue(5));
  });

  it("persists an edited value on save", async () => {
    let sent: Record<string, unknown> | null = null;
    server.use(
      http.get(LOOP_URL, () => HttpResponse.json(makeLoop(3))),
      http.put(LOOP_URL, async ({ request }) => {
        sent = (await request.json()) as Record<string, unknown>;
        return HttpResponse.json(makeLoop(7));
      }),
    );
    renderDialog();
    ...
    expect(sent!.max_blocked_on_human).toBe(7);
  });

  it("sends 0 rather than dropping it — 0 is the documented opt-out", async () => {
    let sent: Record<string, unknown> | null = null;
    server.use(
      http.get(LOOP_URL, () => HttpResponse.json(makeLoop(3))),
      http.put(LOOP_URL, async ({ request }) => {
        sent = (await request.json()) as Record<string, unknown>;
        return HttpResponse.json(makeLoop(0));
      }),
    );
    renderDialog();
    ...
    await waitFor(() => expect(sent).not.toBeNull());
    expect(sent!.max_blocked_on_human).toBe(0);
  });
});
