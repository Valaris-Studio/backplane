...
router = APIRouter(
    prefix="/api/workspaces/{slug}/boards/{board_id}/resources",
    tags=["board-resources"],
)


@router.get("", response_model=list[ResourceRead])
async def list_board_resources(
...
async def list_board_tags(
    board_id: uuid.UUID = Depends(resolve_board_id),
    ctx: WorkspaceContext = Depends(get_workspace),
    db: AsyncSession = Depends(get_db),
):
    service = ResourceService(db)
    return await service.list_tags(ctx.workspace.id, board_id)


@router.post("", response_model=ResourceRead, status_code=201)
async def create_board_resource(
    data: ResourceCreate,
    board_id: uuid.UUID = Depends(resolve_board_id),
    ...
    return await service.create_resource(
        ctx.workspace.id, data, ctx.user.id, board_id
    )
...
async def update_board_resource(
    resource_id: uuid.UUID,
    data: ResourceUpdate,
    board_id: uuid.UUID = Depends(resolve_board_id),
    ctx: WorkspaceContext = Depends(get_workspace_member),
...
async def delete_board_resource(
    resource_id: uuid.UUID,
    board_id: uuid.UUID = Depends(resolve_board_id),
    ctx: WorkspaceContext = Depends(get_workspace_member),
    db: AsyncSession = Depends(get_db),
